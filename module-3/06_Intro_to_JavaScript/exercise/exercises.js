...
function hasTeen(x, y, z) {
	let teen = { from: 13, to: 19 }
	if ((x >= 13 && x <= 19) || (z >= 13 && z <= 19) || (y >= 13 && y <= 19)) {
		return true;
	} else { return false; }
}
/*
...
function lastDigit(x, y) {
	const lastX = String(x).slice(-1);
	const lastY = String(y).slice(-1);
	if (lastX === lastY) {
		return true;
	} else { return false; }
}
...
function seeColor(myString) {
	if (myString.substring(0, 3) == 'red') {
		myString = 'red';
		return myString;
	} else if (myString.substring(0, 4) == 'blue') {
		myString = 'blue';
		return myString;
	} else myString = '';
	return myString;
...
function frontAgain(someString) {
	front = someString.substring(0, 2);
	end = someString.substring(someString.length - 2)
	if (front == end) {
		return true;
	} else { return false; }
}
...
function cigarParty(numCigars, isWeekend) {
	if ((!isWeekend && (numCigars >= 40 && numCigars <= 60)) || (isWeekend && (numCigars >= 40))) {
		return true;
	} else { return false; }
}
/*
...
function fizzBuzz(myNum) {
	if ((myNum % 3 == 0) && (myNum % 5 == 0)) { return 'FizzBuzz' }
	else if (myNum % 3 == 0) { return 'Fizz' }
	else if (myNum % 5 == 0) { return 'Buzz' }
	else return myNum;
}
/*
