...
export default {
  ...
  created() {
    const selectedProductID = this.$route.params.id; //getting off the route object - route object is the state of the current route
    this.$store.commit("SET_ACTIVE_PRODUCT", parseInt(selectedProductID)); //putting into the store
  },
  computed: {
    product() {
      const selectedProductID = this.$store.state.activeProduct;
      return this.$store.state.products.find((p) => p.id === selectedProductID); // p meaning product in this case
    },
  },
};
</script>
